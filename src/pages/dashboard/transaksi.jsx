...
export function Transaksi() {
  ...
  const handleShowOrderDetail = (orderId) => {
   ...
   const OrderDetail = ({ order }) => {
    ...
    const products = order.products ?? []; // menggunakan nullish coalescing untuk mengatasi null
    return (
      <div>
        <h2>Detail Order #{order.id}</h2>
        <p>Tanggal: {format(new Date(order.createdAt), 'dd/MM/yyyy')}</p>
        <p>Customer: {order.customerName}</p>
        <p>Jumlah Item: {order.productCount}</p>
        <p>Total: {order.total.toLocaleString('id-ID', { style: 'currency', currency: 'IDR' })}</p>
        <h3>Produk yang Dibeli:</h3>
        <ul>
          {products.map((product) => ( // menggunakan products.map
